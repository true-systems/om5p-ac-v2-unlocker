...
//	athrs17_reg_write(S17_SGMII_CTRL_REG , 0xc70167de); //SGMII control 
...
#else
//		athrs17_reg_write(S17_P0PAD_MODE_REG  , 0x87680000); //RGMII MAC0/MAC6 change,rgmii_txclk_delay_en,rgmii_rxclk_delay_en,
		athrs17_reg_write(S17_P0PAD_MODE_REG,	0x07680000);
#endif
//		athrs17_reg_write(S17_P5PAD_MODE_REG  , 0x01000000); //rgmii_rxclk_delay_en
...
#if 0
        if ((athrs17_reg_read(S17_MASK_CTRL_REG) & 0xffff) == 0x1302) {
		printf("%s: QCA8337 ID=%x\n",__func__,athrs17_reg_read(S17_MASK_CTRL_REG)); 
